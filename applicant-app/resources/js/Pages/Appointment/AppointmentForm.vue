...
watch(selectedDateValue, (newDate) => {
    if (newDate && isWeekday(newDate)) {
        const reservedSlots = fetchReservedTimeSlots(newDate);
        availableTimeSlots.value = generateTimeSlots(reservedSlots);
        form.date = newDate instanceof Date ? newDate.toISOString().split('T')[0] : newDate;
    } else {
        ...
        <template #content>
            <form @submit.prevent="submitForm">
                ...
                <div class="field mt-4">
                    ...
                    <Select id="office"v-model="selectedOffice" :options="offices" optionLabel="name" placeholder="Select a City" class="w-full" />
                </div>
                <div class="field mt-4">
                ...
                <div v-if="availableTimeSlots.length > 0" class="field mt-4">
                    ...
                    <div class="grid">
                        <div v-for="slot in availableTimeSlots" :key="slot.label"
                             class="col-6 sm:col-4 md:col-3 lg:col-2">
                            <Button
                                :label="slot.label"
